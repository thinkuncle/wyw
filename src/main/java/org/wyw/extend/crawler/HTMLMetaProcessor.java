...
/**
 * 解析元数据指令.
 * 从DOM trees解析元指令解析元指令从DOM树.
 * 处理特定的机器META directives(all, none, nofollow,noindex) 
 * 找到Href tags, HTTP-EQUIV no-cache命令，
 * 所有的元数据 都存储在一个HTMLMetaTags
 * @author wanghaibo
 */
							...
							if (metaTags.getRefresh() && idx != -1) { // set the URL
								...
								if (idx != -1) {
									...
									try {
										...
										/**
										 * XXX 根据规范,这是一个绝对url。 但许多网站使用相对url,指望浏览器处理。
										 * 不幸的是,在某些情况下这可能创建一个无限递归路径(履带陷阱)…
										 * 如果 (!url.startsWith(“/”))url = " + url;
										 */
										try {
