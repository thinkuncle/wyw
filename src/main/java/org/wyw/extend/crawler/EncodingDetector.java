...
public class EncodingDetector {
	...
	public void autoDetectClues(Page fetchResult) {
		String encoding = parseCharacterEncoding(fetchResult.getFromHeaders("Content-Type"));
	 ...
	 *            enough confidence. Note that this will <b>not</b> be
	 *            normalized with {@link EncodingDetector#resolveEncodingAlias}
	 * 
	 * @return Guessed encoding or defaultValue
	 */
	public String guessEncoding(String defaultValue) {
		return clues.size() > 0 ? clues.get(0).value.toLowerCase()
	...
	public static String resolveEncodingAlias(String encoding) {
		try {
			...
			return ALIASES.containsKey(canonicalName) ? ALIASES
					.get(canonicalName) : canonicalName;
		} catch (Exception e) {
			logger.warn("Invalid encoding " + encoding
					+ " detected, using default.");
			return null;
		}
	}
	 ...
	 * detector = new EncodingDetector(NutchConfiguration.create());
	 * 
	 ...
	 * 
	 * byte[] data = ostr.toByteArray(); MimeUtil mimeTypes = new
	 * MimeUtil(conf);
	 * 
